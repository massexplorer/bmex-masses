...
def isotopic(quantity, model, colorbar, wigner, Z, N, A, view_range, uncertainties, even_even):
    ...
    for i in range(len(Z)):
        result = bmex.IsotopicChain(Z[i], model[i], quantity, wigner[i])
...
def isotonic(quantity, model, colorbar, wigner, Z, N, A, view_range, uncertainties, even_even):
    ...
    for i in range(len(N)):
        result = bmex.IsotonicChain(N[i],model[i],quantity,wigner[i])
        if isinstance(result, tuple):
            df = result[0]
        else:
            df = result
...
def isobaric(quantity, model, colorbar, wigner, N, Z, A, view_range, uncertainties, even_even):
    ...
    for i in range(len(A)):
        result = bmex.IsobaricChain(A[i],model[i],quantity,wigner[i])
        if isinstance(result, tuple):
            df = result[0]
        else:
            df = result
...
def isotonic_diff(quantity, model, colorbar, wigner, Z, N, A, view_range, uncertainties, even_even):
    ...
    traces = []
    for i in range(len(N)):
        result_exp = bmex.IsotonicChain(N[i],'AME2020',quantity,wigner[i])
        if isinstance(result_exp, tuple):
            exp = result_exp[0]
        else:
            exp = result_exp
        exp = exp.sort_values(by=['Z'])
